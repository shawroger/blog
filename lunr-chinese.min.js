		...
		function (e, t, n) {
			...
			!(function () {
				...
				(o.version = "0.7.2"),
					...
					(o.tokenizer.seperator = !1),
					(o.tokenizer.separator = /[\s\-]+/),
					(o.tokenizer.load = function (e) {
						var t = this.registeredFunctions[e];
						if (!t) throw new Error("Cannot load un-registered function: " + e);
						return t;
					}),
					(o.tokenizer.label = "default"),
					(o.tokenizer.registeredFunctions = { default: o.tokenizer }),
					(o.tokenizer.registerFunction = function (e, t) {
						t in this.registeredFunctions &&
							o.utils.warn("Overwriting existing tokenizer: " + t),
							(e.label = t),
					...
					(o.Pipeline.registeredFunctions = {}),
					...
					(o.Pipeline.prototype.run = function (e) {
						for (
							...
							for (
								var o = e[r], s = 0;
								s < i && void 0 !== (o = this._stack[s](o, r, e)) && "" !== o;
								s++
							);
							void 0 !== o && "" !== o && t.push(o);
						}
					...
					(o.Vector.prototype.insert = function (e, t) {
						...
						var n = this.list;
						if (!n)
							return (this.list = new o.Vector.Node(e, t, n)), this.length++;
						if (e < n.idx)
							return (this.list = new o.Vector.Node(e, t, n)), this.length++;
					 ...
					 */ (o.Index = function () {
						(this._fields = []),
							...
							(this.documentStore = new o.Store()),
							(this.tokenStore = new o.TokenStore()),
							(this.corpusTokens = new o.SortedSet()),
							(this.eventEmitter = new o.EventEmitter()),
							(this.tokenizerFn = o.tokenizer),
							(this._idfCache = {}),
							this.on(
								"add",
								"remove",
					...
					(o.Index.load = function (e) {
						...
						return (
							(t._fields = e.fields),
							(t._ref = e.ref),
							t.tokenizer(o.tokenizer.load(e.tokenizer)),
							(t.documentStore = o.Store.load(e.documentStore)),
							(t.tokenStore = o.TokenStore.load(e.tokenStore)),
							(t.corpusTokens = o.SortedSet.load(e.corpusTokens)),
							(t.pipeline = o.Pipeline.load(e.pipeline)),
							t
						);
					}),
					...
					(o.Index.prototype.tokenizer = function (e) {
						return (
							(e.label && e.label in o.tokenizer.registeredFunctions) ||
								o.utils.warn(
						...
						for (var s = 0; s < i.length; s++) {
							for (
								...
								var h = this._fields[l],
									...
									f = c.length;
								if (f) {
									for (var d = 0, p = 0; p < f; p++) c[p] === a && d++;
									u += (d / f) * h.boost;
								}
							}
							this.tokenStore.add(a, { ref: r, tf: u });
						}
						...
						return t.some(function (e) {
							...
							? (t.forEach(function (e, t, s) {
									var a = (1 / s.length) * this._fields.length * r,
										u = this,
										l = this.tokenStore.expand(e).reduce(function (t, i) {
											var r = u.corpusTokens.indexOf(i),
												s = u.idf(i),
												l = 1,
												h = new o.SortedSet();
											if (i !== e) {
												var c = Math.max(3, i.length - e.length);
												l = 1 / Math.log(c);
											}
											r > -1 && n.insert(r, a * s * l);
											for (
												var f = u.tokenStore.get(i),
													d = Object.keys(f),
													p = d.length,
													v = 0;
												v < p;
					...
					(o.Store.load = function (e) {
						...
						return (
							(t.length = e.length),
							(t.store = Object.keys(e.store).reduce(function (t, n) {
					 ...
					 */ (o.stemmer = (function () {
						var e = {
							...
							x = /^(.+?)e$/,
							k = /ll$/,
							b = new RegExp("^" + i + n + "[^aeiouwxy]$");
						return function (n) {
							var i, E, _, F, z, O, P;
							if (n.length < 3) return n;
							if (
								((_ = n.substr(0, 1)),
								"y" == _ && (n = _.toUpperCase() + n.substr(1)),
								(F = u),
								(z = l),
								F.test(n)
									? (n = n.replace(F, "$1$2"))
									: z.test(n) && (n = n.replace(z, "$1$2")),
								(F = h),
								(z = c),
								F.test(n))
							) {
								var T = F.exec(n);
								(F = r), F.test(T[1]) && ((F = f), (n = n.replace(F, "")));
							} else if (z.test(n)) {
								var T = z.exec(n);
								(i = T[1]),
									(z = a),
							...
							if (((F = g), F.test(n))) {
								var T = F.exec(n);
								(i = T[1]), (E = T[2]), (F = r), F.test(i) && (n = i + e[E]);
							}
							if (((F = y), F.test(n))) {
								var T = F.exec(n);
								(i = T[1]), (E = T[2]), (F = r), F.test(i) && (n = i + t[E]);
							}
							if (((F = S), (z = w), F.test(n))) {
								var T = F.exec(n);
								(i = T[1]), (F = o), F.test(i) && (n = i);
							} else if (z.test(n)) {
								var T = z.exec(n);
								(i = T[1] + T[2]), (z = o), z.test(i) && (n = i);
							}
							if (((F = x), F.test(n))) {
								var T = F.exec(n);
								(i = T[1]),
									...
									(z = s),
									(O = b),
									(F.test(i) || (z.test(i) && !O.test(i))) && (n = i);
							}
							return (
								(F = k),
								(z = o),
								F.test(n) && z.test(n) && ((F = f), (n = n.replace(F, ""))),
								"y" == _ && (n = _.toLowerCase() + n.substr(1)),
								n
							);
						};
					})()),
...
(function () {
	var docs = Object.values(window.searchIndex.documentStore.docs).filter(
		function (it) {
			for (let key of window.$ZOLA_CONFIG.navs) {
	...
	function getList(result) {
		return result.map(function (it) {
			var aim = docs.filter(function (v) {
				return v.id == it.ref;
			});
			if (aim.length > 0) {
				it.title = aim[0].title;
			}
			return it;
		});
	}
	...
	new window.Vue({
		...
		methods: {
			search() {
				var result = idx.search(this.text);
				this.result = getList(result);
			},
			scoreShow(score) {
				var s = (Math.round(Number(score) * 10000) / 100).toString();
				if (s.indexOf(".") == -1) {
					s = s + ".00";
				} else if (2 + s.indexOf(".") == s.length) {
					s = s + "0";
				}
				return s;
			},
			renderText(text) {
				if (window.$phonita && window.$phonita.checkText) {
					return window.$phonita.checkText(text).text;
				} else {
					return text;
				}
			},
